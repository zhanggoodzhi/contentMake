    ...
    <div v-bind:class="ifBeChoose? 'questionSelect' : 'question' ">
      <div class="question-title">
        <div
          ...
          @click="CopyClick"
        >
          <i class="font_family icon-copy"></i>
          复制
        </div>
        <div
          v-if="ifBeChoose"
        ...
        >
          ...
          <i class="  font_family icon-select"></i>
          取消选取
        </div>
        <div
          v-else
      ...
      <div class="question-content">
        <div v-if="questionData.ImagePath!=null && questionData.ImagePath.length> 0">
          <img
            :src="getSliceUrl(questionData.ImagePath)"
            alt=""
            style="width: 330px;"
          >
        </div>
        <div v-if="questionData.Content && questionData.Content.length">
          <div v-for="cont in questionData.Content">
            <!-- text如何显示 显示文字、图片、视频 -->
            <div
              v-if="cont.ImagePath && cont.ImagePath!=''"
              class="text"
            >
              <img
                :src="getSliceUrl(cont.ImagePath)"
                alt=""
                 style="width: 330px;"
              >
            </div>
        ...
        <div
          class="reftextDiv edit-paper-area-basket-collapse"
...
export default {
  ...
  computed: {
    ...
    // 判断是否被选取
    ifBeChoose() {
      for (const v of this.paperData.PaperAreaViewModels[this.areaIndex]
  ...
  methods: {
    ...
    QuestionCopy(type) {
      ...
      if (type == "3") {
        ...
        const areaType = newData.PaperAreaViewModels[this.areaIndex].AreaType;
        if (areaType == 25) {
          newData.PaperAreaViewModels[
            this.areaIndex
          ].PaperQuestionViewModels[0] =
            newData.PaperAreaViewModels[
              this.areaIndex
            ].PaperQuestionViewModels[0].savedQuestionData;
          /* const getRelatedQuestionIndexTwo = this.getRelatedQuestionIndexTwo();   */

          newData.PaperAreaViewModels[
            this.areaIndex
          ].PaperQuestionViewModels[1] =
            newData.PaperAreaViewModels[
              this.areaIndex
            ].PaperQuestionViewModels[1].savedQuestionData;
        } else {
          newData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
        ...
        DuplicateQuestion(param).then(d => {
          ...
          const responseData = d.ResponseData;
          const areaType = this.paperData.PaperAreaViewModels[this.areaIndex]
            .AreaType;
          if (areaType == 25) {
            //听后记录并转述特殊处理(只会有两个小题)
            
            this.ReplaceQuestion(0, qData.PaperQuestion, responseData, type);
            this.ReplaceQuestion(1, qData.PaperQuestionTwo, responseData, type);
          } else {
    ...
    //复制Question 并替换当前试卷试题的相关ID
    ReplaceQuestion(questionIdx, CopyPaperQuestion, responseData, type) {
      const nData = deepClone(this.paperData);
      const areatype = nData.PaperAreaViewModels[this.areaIndex].AreaType;
      //之前的questionid不能变
      nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
        questionIdx
      ] = CopyPaperQuestion;
      // 将初始数据保存下来，取消选取用
      nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
        questionIdx
      ].savedQuestionData = this.paperData.PaperAreaViewModels[
        this.areaIndex
      ].PaperQuestionViewModels[questionIdx];
      //开始替换当前试卷之前的ID
      nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
        questionIdx
      ].QuestionID = this.paperData.PaperAreaViewModels[
        this.areaIndex
      ].PaperQuestionViewModels[questionIdx].QuestionID;
      nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
        questionIdx
      ].PaperID = this.paperData.PaperAreaViewModels[
        this.areaIndex
      ].PaperQuestionViewModels[questionIdx].PaperID;
      nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
        questionIdx
      ].AreaID = this.paperData.PaperAreaViewModels[
        this.areaIndex
      ].PaperQuestionViewModels[questionIdx].AreaID;
      //替换之前的资源文件id
      const audioid =
        nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
          questionIdx
      ...
      nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
        questionIdx
      ].AudioID = this.GetNewResourceID(audioid, responseData);
      nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
        questionIdx
      ].AudioPath = this.GetNewResourcePath(audioid, responseData);
      const iamgeid =
        nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
          questionIdx
        ].ImageID;
      nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
        questionIdx
      ].ImageID = this.GetNewResourceID(iamgeid, responseData);
      nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
        questionIdx
      ].ImagePath = this.GetNewResourcePath(iamgeid, responseData);
      const promptaudioid =
        nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
          questionIdx
        ].PromptAudioID;
      nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
        questionIdx
      ].PromptAudioID = this.GetNewResourceID(promptaudioid, responseData);
      nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
        questionIdx
      ].PromptAudioPath = this.GetNewResourcePath(promptaudioid, responseData);
      const tipsaudioid =
        nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
          questionIdx
        ].TipsAudioID;
      nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
        questionIdx
      ].TipsAudioID = this.GetNewResourceID(tipsaudioid, responseData);
      nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
        questionIdx
      ].TipsAudioPath = this.GetNewResourcePath(tipsaudioid, responseData);
      //遍历content
      nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
        questionIdx
      ].PaperContentViewModels.forEach((content, index) => {
        //获取原来的content
        //弃用Idx来查找content 这样会因为Idx不一致导致无法复制或者选取  只根据数组的顺序来复制和选取
        var oldcontent = this.GetOldContent(questionIdx,index); //this.GetOldContent(content.Idx);
        if (oldcontent != null) {
          content.ContentID = oldcontent.ContentID;
          content.PaperID = oldcontent.PaperID;
          content.QuestionID = oldcontent.QuestionID;
          ...
          nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
            questionIdx
          ].PaperContentViewModels[index] = content;
        }
        console.log(nData);
      });
      //如果是选取
      if (type == "2") {
        nData.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
          questionIdx
        ].TrackQuestionID = this.questionData.PaperQuestion.QuestionID;
      }
      this.changePaperData(this.addIfImageData(nData));
    },
    ...
    GetOldContent(questionIndex,index) {
      const data = deepClone(this.paperData);
      if (index < data.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[questionIndex].PaperContentViewModels.length) {
        return data.PaperAreaViewModels[this.areaIndex].PaperQuestionViewModels[
          questionIndex
        ].PaperContentViewModels[index];
      } else {
        //返回空的
        return null;
      }
    ...
    getRelatedQuestionIndex() {
      for (
        let i = 0;
        i <
        this.paperData.PaperAreaViewModels[this.areaIndex]
          .PaperQuestionViewModels.length;
        i++
      ) {
        if (
          this.paperData.PaperAreaViewModels[this.areaIndex]
            .PaperQuestionViewModels[i].TrackQuestionID ===
          this.questionData.PaperQuestion.QuestionID
        ) {
          return i;
        }
      }
    },
    getRelatedQuestionIndexTwo() {
      for (
        let i = 0;
        i <
        this.paperData.PaperAreaViewModels[this.areaIndex]
        ...
        if (
          this.paperData.PaperAreaViewModels[this.areaIndex]
            .PaperQuestionViewModels[i].TrackQuestionID ===
          this.questionData.PaperQuestionTwo.QuestionID
        ) {
          return i;
        }
      }
...
.questionSelect {
  ...
  .question-content {
    ...
    .text {
      ...
      font-size: 16px;
      line-height: 28px;
      color: #999999;border-bottom: solid 1px #ddd;
    }
    .option {
      font-family: ArialMT;
      font-size: 14px;
...
<style lang="less">
.edit-paper-area-basket-collapse {
