      ...
      <template v-else>
        <div>
          <div
            v-for="(ssv,ssi) in sv.PaperContentViewModels"
            :key="ssv.ContentID"
            class="content-main clearfix mt"
          >
            <p class="font14 prompt">{{sv.Prompt}}</p>
            <InputBox
              :disabled="ifBeChoose(sv)"
              :error="getEditPaperValidateError('ContentTips',aIndex,si,ssi)"
              @focus="()=>{selectQuestion(si);editPaperValidateError('ContentTips', '', aIndex, si,ssi);}"
              class="mt"
              :textarea="true"
              :inputValue="ssv.Tips"
              :audioMode="true"
              :audioUrl="getSliceUrl(ssv.TipsAudioPath)"
              @delAudio="delContentAudio(si,ssi)"
              @uploadAudioSuccess="($event)=>{uploadContentAudioSuccess(si,ssi,$event);editPaperValidateError('ContentTips', '', aIndex, si,ssi);}"
              placeholder="答案提示"
...
export default {
  ...
  methods: {
    ...
    batchAddOk(si, ssi, value) {
      ...
      newData.PaperAreaViewModels[this.aIndex].PaperQuestionViewModels[si].PaperContentViewModels[ssi].PaperContentAnswerViewModels = removeNoneArr; //PaperContentAnswerViewModels.concat(removeNoneArr);
      this.changePaperData(newData);
    },
    ...
    async delContentAudio(si, ssi) {
      this.pushDeleteMedia(this.area.PaperQuestionViewModels[si].PaperContentViewModels[ssi].TipsAudioID);
      this.changeEditPaperContentProp(this.aIndex, si, ssi, 'TipsAudioID', '');
      this.changeEditPaperContentProp(this.aIndex, si, ssi, 'TipsAudioPath', '');
    },
    uploadContentAudioSuccess(si, ssi, ref) {
      this.changeEditPaperContentProp(this.aIndex, si, ssi, 'TipsAudioID', ref.ResourceID);
      this.changeEditPaperContentProp(this.aIndex, si, ssi, 'TipsAudioPath', ref.ResourcePath);
    },
    ifBeChoose(sv) {
