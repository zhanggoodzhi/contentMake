<template>
  <div>
    ...
    >
      ...
      <div class="tc">
        <p class="font14">
          该模板已被<span class="orange"> {{relationNumber}} </span>套试卷使用，
          你确定要编辑模板内容吗？
        </p>
      </div>
    </Modal>
...
import TemplateInfoItem from '@/components/templateComplete/TemplateInfoItem';
import { DeleteTemplate, GetTemplateList, GetTemplate, UpdateTemplateName } from "@/plugins/service";
export default {
  components: {
    TemplateInfoItem
  },
  ...
  methods: {
    ...
    editTemplate(id, type, relationNumber) {
      // type:lock 表示锁住题型，type:prop表示只能编辑属性
      if (type === 'lock') {
